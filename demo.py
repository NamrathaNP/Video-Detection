...
class RandomThread(Thread):
    ...
    def get_frame(self):
        ...
        ret,frame = cap.read()
        cv2.imwrite('./templates/test.jpg',frame)
            ...
            if not to.counted:
                ...
                if (direction < 0 and centroid[0] <=50) or (direction>0 and centroid[0] >=350):
                    ...
                    if gender is 'Male':
                        ...
                        socketio.emit('newnumber', {'data1': data1, 'data2': data2,'display':"HI 3"} , namespace='/test')
                        ...
                        socketio.emit('newnumber', {'data1': data1, 'data2': data2,'display':"HI 4"} , namespace='/test')
...
def chart():
    ...
    print(totalMale)
    socketio.emit('newframe', {'hourly_count_dict_male': hourly_count_dict_male, 'hourly_count_dict_female': hourly_count_dict_female,'totalMale':totalMale, 'totalFemale':totalFemale , 'frame':frame} , namespace='/test')
